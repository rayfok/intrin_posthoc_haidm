...
def export_data():
    ...
    with app.app_context():
        with open(os.path.join(dir_path, "haidm_task_responses.json"), "w") as out:
            json.dump([dataclasses.asdict(r) for r in Response.query.all()], out, cls=DateTimeEncoder)

        with open(os.path.join(dir_path, "haidm_survey_responses.json"), "w") as out:
            json.dump([dataclasses.asdict(r) for r in ExitSurveyResponse.query.all()], out, cls=DateTimeEncoder)
